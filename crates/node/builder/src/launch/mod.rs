...
use reth_node_api::{
    EngineTypes, FullNodeComponents, FullNodeTypes, NodeAddOns, NodeTypesWithDB, NodeTypesWithEngine
};
use reth_node_core::{
...
use reth_db_api::database_metrics::{DatabaseMetadata, DatabaseMetrics};
...
pub trait LaunchSeqeuncer<Target> {
    type Node;

    fn launch_sequencer(self, target: Target) ->  impl Future<Output = eyre::Result<RpcServerHandle>> + Send;

}

pub trait LaunchDevSequencer<Target>{
    type Node;

    fn launch_dev_sequencer(self,  target: Target) -> impl Future<Output = eyre::Result<Self::Node>> + Send;
}
    ...
    async fn launch_node(
        ...
        let ctx = ctx
            ...
            .with_blockchain_db::<T, _>(move |provider_factory| {
                let z = BlockchainProvider::new(provider_factory, tree);
                Ok(z?)
        ...
        let mut max_block = ctx.max_block(network_client.clone()).await?;
        max_block = Some(64);
        ...
        println!("StaticFileProducer initialized");
        ...
        let (pipeline, client) = if !_flag {
            println!( "Starting Reth in dev mode");

            for (idx, (address, alloc)) in ctx.chain_spec().genesis().alloc.iter().enumerate() {
                println!("Allocated Genesis Account: {:02}. {} ({} ETH)", idx, address.to_string(), format_ether(alloc.balance));
            }

            // install auto-seal
            let mining_mode =
                ctx.dev_mining_mode(ctx.components().pool().pending_transactions_listener());
            println!( "configuring dev mining mode");
        ...
        if let Some(rpc_ws_url) = ctx.node_config().debug.rpc_consensus_ws.clone() {
            println!( "Using rpc provider as consensus client");
...
impl<Types, T, CB, AO> LaunchSeqeuncer<NodeBuilderWithComponents<T, CB, AO>> for DefaultNodeLauncher
where
    Types: NodeTypesWithDB<ChainSpec: EthereumHardforks + EthChainSpec> + NodeTypesWithEngine,
    T: FullNodeTypes<Provider = BlockchainProvider<Types>, Types = Types>,
    CB: NodeComponentsBuilder<T>,
...
async fn launch_sequencer(self, target: NodeBuilderWithComponents<T, CB, AO>) ->  eyre::Result<RpcServerHandle> {
    let Self { ctx } = self;
    let NodeBuilderWithComponents {
        adapter: NodeTypesAdapter { database },
        components_builder,
    ...
    // create pipeline
    let network_client = ctx.components().network().fetch_client().await?;
    let (consensus_engine_tx, consensus_engine_rx) = unbounded_channel::<BeaconEngineMessage<Types::Engine>>();
    ...
    // Start RPC servers
    let (rpc_server_handle, rpc_registry) = crate::rpc::launch_without_engine_servers(
        ctx.node_adapter().clone(),
        ctx.node_config(),
        rpc,
    )
